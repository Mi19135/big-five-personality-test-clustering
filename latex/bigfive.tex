...
\begin{itemize}
    ...
    \item interpretirati dobijene klastere.
\end{itemize}
...
Upitnik se sastoji od 50 tvrdnji, po 10 za svaku od vec navednih pet dimenzija ličnosti.
...
\begin{itemize}
    ...
    \item DBSCAN
    \item Probabilističko klasterovanje zasnovano na mešavini normalnih raspodela - GMM
...
Primetno je da uključivanje vremenskih promenljivih (\_E) ne menja značajno raspodelu klastera.
...
\paragraph{Zaključak aglomerativne metode} 
 Rezultati pokazuju značajne razlike u stabilnosti i kvalitetu formiranih klastera.
...
\begin{figure}[H]
    ...
    \caption{Spectral clustering ($k=5$ , $n\_neighbors=20$) za različite veličine uzorka – poređenje bez i sa vremenskim promenljivama}
...
Kod modela bez \_E atributa,za manje vrednosti parametra $\gamma$ (0.1 i 0.25), vizuelna analiza ukazuje na blagu tendenciju segmentacije prostora u nekoliko vecih grupa.Medjutim ova struktura nije stabilna kroz promene parametra $\gamma$.
 
Sa povećanjem $\gamma$ dolazi do potpunog kolapsa modela, gde gotovo svi slogovi bivaju dodeljeni
...
\subsection{Redukcija dimenzionalnosti (UMAP)}
...
\subsection{Analiza trodimenzionalnih kombinacija subskala}
...
\subsection{K-means klasterovanje nad svih pet subskala}

Klaster analiza je sprovedena nad pet sumiranih i standardizovanih subskala (EXT, EST, AGR, CSN i OPN). Nakon standardizacije podataka, izdvojen je uzorak od 15000 ispitanika radi stabilnosti i računske efikasnosti analize. Primenjen je K-means algoritam sa unapred definisanim brojem klastera $k=3$.
...
\subsection*{K-means}
Evaluacija klastera na uzorku od 15 000 ispitanika pokazuje niske vrednosti
Silhouette indeksa (~0.05) i visoke Davies-Bouldin vrednosti, što ukazuje na
slabu separaciju i značajno preklapanje klastera. Uključivanje vremenskih
promenljivih (\_E) ne dovodi do poboljšanja kvaliteta klasterske strukture.

\subsection*{Hijerarhijsko klasterovanje}

Rezultati hijerarhijskog pristupa bili su bez jasne i stabilne segmentacije. Formirani klasteri pokazivali su značajno preklapanje i odsustvo izraženih granica između grupa.


\subsection*{DBSCAN}

Ovaj algoritam u najvećoj meri identifikuje jednu dominantnu grupu, dok ostatak tačaka često klasifikuje kao šum (noise), u zavisnosti od izabranih parametara ($\varepsilon$ i \textit{min\_samples}).

Nije uočena stabilna segmentacija u više jasno diferenciranih klastera, već se struktura podataka ponovo pokazala kao kompaktna i kontinuirana, bez izražene gustinske separacije.

\subsection*{Gaussian Mixture Model (GMM)}
Kod modela bez \_E atributa komponente su bile približno ujednačene, ali uz značajno preklapanje i vrlo niske vrednosti Silhouette indeksa.

Kod modela sa \_E atributima dolazi do izraženog kolapsa komponenti pri većim uzorcima, gde jedna komponenta obuhvata gotovo sve instance.

\subsection*{Spectral clustering}


Primena spektralnog pristupa, koji je pogodan za nelinearne strukture, nije dovela do značajno boljeg razdvajanja klastera u odnosu na prethodne metode.

\subsection*{Dimenzionalna redukcija (PCA i UMAP)}

PCA projekcije nisu pokazale jasno odvojene grupe u nižedimenzionalnom prostoru. UMAP, kao nelinearan metod, takođe nije otkrio izražene separacije, već je potvrdio postojanje kompaktne, kontinuirane distribucije podataka bez jasnih klasterskih granica.

\section{Opšti zaključak}

Uporedna analiza svih primenjenih metoda pokazuje visok stepen konzistentnosti rezultata. Bez obzira na metodološki pristup, nije identifikovana stabilna i interpretabilna klasterska struktura.

Dobijeni nalazi ukazuju da podaci ne poseduju izraženu multimodalnost, već da pre predstavljaju kontinuum osobina. Uočene segmentacije uglavnom su rezultat izolacije ekstremnih vrednosti, a ne prirodno formiranih grupa.
